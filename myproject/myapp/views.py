...
from myapp.models import userinfo,Userdata,Messages,Friends,Signup
...
#				 with connection.cursor() as cursor:
#				  cursor.execute(query,params)
				 user=Userdata(id=id,username=request.session['username'],emailid=email_id,password=passwd,status='accepted',image=img)
				 user.save()
				 dbuser_pro=Userdata.objects.raw("select a.* from myapp_userdata a where a.username=%s and a.status='accepted'",[request.session['username']])
